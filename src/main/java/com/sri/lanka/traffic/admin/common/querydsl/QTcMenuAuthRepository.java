...
public class QTcMenuAuthRepository {
	...
	public List<TcMenuAuthDTO> getSideAuthMenuList(String authgrpId) {
		List<TcMenuAuthDTO> result = queryFactory
				.select(Projections.bean(TcMenuAuthDTO.class, tcMenuAuth.authgrpId, parentTcMenuMng.menuId, parentTcMenuMng.menuCd,
				...
				.on(parentTcMenuMng.menuId.eq(tcMenuAuth.menuId).and(tcMenuAuth.authgrpId.eq(authgrpId)))
		...
		if (!CommonUtils.isNull(result)) {
			result.stream().filter(x -> !CommonUtils.isNull(x))
					.forEach(parentMenuList -> parentMenuList.setSubMenuList(queryFactory
							.select(Projections.bean(SubAuthMenuInfo.class, tcMenuAuth.authgrpId, tcMenuMng.menuId, tcMenuMng.menuCd,
	...
	public List<TcMenuMngMenuCheckDTO> getAuthMenuListByAuthority(TcMenuAuth menuAuth) {
		List<TcMenuMngMenuCheckDTO> result = queryFactory
				...
				.where(tcMenuMng.uppermenuCd.isNotNull().and(tcMenuAuth.authgrpId.eq(menuAuth.getAuthgrpId()))
