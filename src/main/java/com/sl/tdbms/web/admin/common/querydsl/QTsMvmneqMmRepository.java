...
public class QTsMvmneqMmRepository {

	final JPAQueryFactory queryFactory;
	
	QTsMvmneqMm tsMvmneqMm = QTsMvmneqMm.tsMvmneqMm;
	...
	public List<PeriodSiteDTO> getSiteIdList(StatisticsByPeriodSerachDTO searchDTO) {
		...
		return queryFactory.select(
				...
				.from(tsMvmneqMm)
				.innerJoin(tlMvmneqCur).on(tsMvmneqMm.instllcId.eq(tlMvmneqCur.instllcId))
				.where(
	...
	private BooleanExpression startDateCondition(String startDate) {
		return startDate != null && !startDate.isEmpty() ? tsMvmneqMm.statsYymm.goe(startDate) : null;
	}

	private BooleanExpression endDateCondition(String endDate) {
		return endDate != null && !endDate.isEmpty() ? tsMvmneqMm.statsYymm.loe(endDate) : null;
	}
	...
	public List<TlPeriodRsltStatisticsDTO.PeriodChartData> getChartData(StatisticsByPeriodSerachDTO searchDTO) {
		...
		return queryFactory.select(Projections.bean(TlPeriodRsltStatisticsDTO.PeriodChartData.class,
						tsMvmneqMm.statsYymm.as("statsDate"),
						tlMvmneqCur.instllcNm.as("instllcNm"),
						tmInstllcRoad.drctCd.as("drctCd"),
						cdNm.as("cdNm"),
						tsMvmneqMm.trfvlm.sum().as("trfvlm"),
						Expressions.numberTemplate(Double.class, "ROUND(AVG({0}), 1)", tsMvmneqMm.avgSpeed).as("avgSpeed")))
				.from(tsMvmneqMm)
				.innerJoin(tlMvmneqCur).on(tsMvmneqMm.instllcId.eq(tlMvmneqCur.instllcId))
				.innerJoin(tmInstllcRoad).on(tlMvmneqCur.instllcId.eq(tmInstllcRoad.instllcId))
				.leftJoin(tcCdGrp).on(tcCdGrp.grpCd.eq("TRF_DRCT_CD"))
				...
				.groupBy(tsMvmneqMm.statsYymm,tlMvmneqCur.instllcNm,tmInstllcRoad.drctCd,cdNm)
				.orderBy(tsMvmneqMm.statsYymm.asc())
				.fetch();
	}
