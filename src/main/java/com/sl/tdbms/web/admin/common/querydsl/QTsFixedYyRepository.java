...
public class QTsFixedYyRepository {
	final JPAQueryFactory queryFactory;

	QTsFixedYy tsFixedYy = QTsFixedYy.tsFixedYy;
	...
	public List<PeriodSiteDTO> getSiteIdList(StatisticsByPeriodSerachDTO searchDTO) {
        ...
        return queryFactory.select(
				...
				.from(tsFixedYy)
				.innerJoin(tlFixedCur).on(tsFixedYy.instllcId.eq(tlFixedCur.instllcId))
				.where(
	...
	private BooleanExpression startDateCondition(String startDate) {
		return startDate != null && !startDate.isEmpty() ? tsFixedYy.statsYy.goe(startDate) : null;
	}

	private BooleanExpression endDateCondition(String endDate) {
		return endDate != null && !endDate.isEmpty() ? tsFixedYy.statsYy.loe(endDate) : null;
	}
	...
	public List<TlPeriodRsltStatisticsDTO.PeriodChartData> getChartData(StatisticsByPeriodSerachDTO searchDTO) {
		...
		return queryFactory.select(Projections.bean(TlPeriodRsltStatisticsDTO.PeriodChartData.class,
						tsFixedYy.statsYy.as("statsDate"),
						tlFixedCur.instllcNm.as("instllcNm"),
						tmInstllcRoad.drctCd.as("drctCd"),
						cdNm.as("cdNm"),
						tsFixedYy.trfvlm.sum().as("trfvlm"),
						Expressions.numberTemplate(Double.class, "ROUND(AVG({0}), 1)", tsFixedYy.avgSpeed).as("avgSpeed")))
				.from(tsFixedYy)
				.innerJoin(tlFixedCur).on(tsFixedYy.instllcId.eq(tlFixedCur.instllcId))
				.innerJoin(tmInstllcRoad).on(tlFixedCur.instllcId.eq(tmInstllcRoad.instllcId))
				.leftJoin(tcCdGrp).on(tcCdGrp.grpCd.eq("TRF_DRCT_CD"))
				...
				.groupBy(tsFixedYy.statsYy,tlFixedCur.instllcNm,tmInstllcRoad.drctCd,cdNm)
				.orderBy(tsFixedYy.statsYy.asc())
				.fetch();
	}
